...
`codex/minter-architecture-plan` design (the 808-line migration plan + setup-actions
reference) and the Track-B harvest decisions recorded in
`planning/tortoise-v1-state-review.md` §8. This supersedes the codex branch; the stale
USDC `TortoiseMintRouter` prototype is left behind there and is **not** carried onto
`main`. See the "Harvest From Track B" section below for what the consolidation added.

This document specifies a custom Tortoise minter that mints In Process/Zora-compatible
ERC-1155 tokens directly via `adminMint`, paying in **native ETH**, with Tortoise's
5/10/85 fee model, multi-recipient splits, non-blocking shell crediting, and ETH
pending-claims. Locked architecture: **immutable** (no proxy), **no premint**,
`collect()` as the sole paid entrypoint. Companion file:
`planning/setup-actions-reference.md`.
...
## Harvest From Track B (InProcess fork)

These items are folded in from the techengme/ziad deployed fork
(`origin/in_process`, `origin/ziad-testing`) per `tortoise-v1-state-review.md` §8.
They are **additive** and never touch the paid `collect()` path, so they do not
compromise the fee/split/shell/pending-claim model. Nothing else from Track B is
adopted (its blocking shell calls, no-deferral, hardcoded 25/75 split, USDC two-token
model, and proxy/upgradeable pattern are explicitly rejected).

### H.1 — Verified InProcess integration facts

Match the interface stubs to the fork's verified `IInProcess1155`
(`in_process:src/in_process/interfaces/IInProcess1155.sol`):

- `adminMint(address,uint256,uint256,bytes)` signature is confirmed (already pinned in
  `setup-actions-reference.md` §C.2).
- `PERMISSION_BIT_MINTER = 4`, `PERMISSION_BIT_ADMIN = 2` confirmed.
- Reward/attribution hooks observed on the creator: `createReferrals`, `firstMinters`,
  `getCreatorRewardRecipient`. The Tortoise minter does **not** depend on these (it owns
  distribution), but deploy/fork tests should be aware they exist.
- Deploy/auth order confirmed: deploy shell → deploy minter → `initialize`/configure →
  register minter as authorized caller on the shell. (Tortoise is immutable, so
  "initialize" is constructor wiring, not a proxy initializer.)

### H.2 — Discovery / recognition shim (no payment sacrifice)

So InProcess's permission system and indexers recognize the contract as a real minter:

- Implement `IMinter1155` + ERC-165 `supportsInterface` returning true for `IMinter1155`
  and `IERC165`.
- Implement `requestMint(...)` as a **hard revert** (`error UseCollectInstead();`). The
  standard 1155 `mint()`→`requestMint`→ProtocolRewards path is deliberately unusable —
  it would route ETH through the 1155 to a single `fundsRecipient` and cannot express
  Tortoise splits/shell/deferral. The revert makes accidental standard-path mints fail
  loudly instead of silently bypassing the fee model. (Pattern from Track B
  `TortoiseMinter.sol:262-280`.)

⚠️ **Open dependency:** the shim only buys *discovery* if InProcess's indexer surfaces
mints performed via `adminMint` (not the standard path). Confirm with the InProcess team
(see "In Process Team Dependency" below). If their indexer only watches standard-path
mints, the shim still earns permission-system recognition but Tortoise discovery relies
entirely on its own event indexer (Phase 4) — which is the assumed baseline anyway.

### H.3 — App-driven sale config via `callSale` (optional, Phase 2)

In addition to the `onlyOwner` `setSale` and the EIP-712 `setSaleWithArtistSignature`
(D.1), optionally accept sale config routed through the collection's `callSale`
(`msg.sender == collection`). This lets artists configure sales from InProcess's existing
app UI. Authorization model: verify the calling collection is Tortoise-managed and that
the ultimate caller holds collection admin; sale config never moves ETH, so this is a
config-surface addition only. **Only build if** InProcess confirms their app drives sale
config via `callSale` on third-party minters; otherwise the EIP-712 path is sufficient
and this is skipped.

### H.4 — Per-wallet cap via Zora's `LimitedMintPerAddress` pattern

Implement the D.3 `maxTokensPerAddress` cap using Zora's canonical
`LimitedMintPerAddress` pattern (keyed `(tokenContract, tokenId, wallet)`,
increment-then-check) rather than a bespoke counter. Same storage shape as D.3's
`mintedByAddress`; this just adopts the audited increment/revert idiom.

### H.5 — Operational scaffolding

From `origin/ziad-testing`:

- Env-driven `script/DeployBaseSepolia.s.sol` (reads addresses/keys from env, fails
  closed on missing config).
- `src/TORSTest.sol` faucet token for testnet TORT staking.
- `addresses/<chainId>.json` registry convention (e.g. `addresses/84532.json`) plus
  chain routing in `script/Config.s.sol`, so deploy scripts resolve per-network
  addresses from one source of truth.
...
- Implement collect flow with native ETH, strict-equality `msg.value`, `mintTo` recipient argument, and per-wallet cap (per H.4 `LimitedMintPerAddress` pattern).
- Implement single `collect`.
- Add the discovery/recognition shim (H.2): `IMinter1155` + `supportsInterface`, and a hard-reverting `requestMint`.
...
- Add `supportsInterface`/`requestMint`-revert tests for the H.2 shim.
- (Optional, H.3) Add `callSale`-routed `setSale` if InProcess confirms their app uses it.
...
- Require an explicit per-network factory allowlist. For Base Sepolia, scripts must
  ...
  deployments.
- Add the H.5 operational scaffolding: env-driven `DeployBaseSepolia.s.sol`, the
...
4. Confirm whether InProcess's indexer/app surfaces mints performed via `adminMint` (vs. only standard-path `mint()`). This determines how much the H.2 discovery shim buys; Tortoise's own indexer (Phase 4) is the baseline regardless.
5. Confirm `totalRewardPct` / `ethRewardAmount` / any protocol mint-fee stay zero for ETH mints, so the 5/10/85 model isn't skimmed.
6. (If pursuing H.3) Confirm whether the InProcess app drives third-party-minter sale config via the collection's `callSale`.
